...
local function PoliceCall()
    ...
    if math.random(1, 100) <= chance then
        TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Ev soygunu", false)
    end
end
...
local function lockpickFinish(success)
    if success then
        TriggerServerEvent('snr:server:enterHouse', closestHouse)
        QBCore.Functions.Notify("İşe Yaradı!", "success", 2500)
    else
        if usingAdvanced then
            local itemInfo = QBCore.Shared.Items["lockpick2"]
            if math.random(1, 100) < 20 then
                TriggerServerEvent("QBCore:Server:RemoveItem", "lockpick2", 1)
        ...
        QBCore.Functions.Notify("İşe Yaramadı..", "error", 2500)
    end
end
...
local function searchCabin(cabin)
    local ped = PlayerPedId()
    local AramaSans = math.random(12000, 20000)
    
    local found, player = GetClosestPlayerMenu()

    if found then
        QBCore.Functions.Notify("Yakınında Biri Varken Bunu Yapamazsın!", "error")
    else
        ...
        LockpickDoorAnim()
        TriggerServerEvent('snr:server:SetBusyState', cabin, currentHouse, true)
        FreezeEntityPosition(ped, true)
        IsLockpicking = true
        QBCore.Functions.Progressbar("preparing_animal", "Arıyorsun...", AramaSans, false, false, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
            ...
            disableCombat = true,
                ...
                openingDoor = false
                ClearPedTasks(PlayerPedId())
                TriggerServerEvent('snr:server:searchCabin', cabin, currentHouse)
                Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
                TriggerServerEvent('snr:server:SetBusyState', cabin, currentHouse, false)
                SucceededAttempts = 0
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('snr:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('snr:client:ResetHouseState', function(house)
...
RegisterNetEvent('snr:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('snr:client:setHouseState', function(house, state)
    Config.Houses[house]["opened"] = state
end)

RegisterNetEvent('snr:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('snr:client:SetBusyState', function(cabin, house, bool)
                            ...
                            if closestHouse ~= nil then
                                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                                    if not Config.Houses[closestHouse]["opened"] then
                                        ...
                                        QBCore.Functions.Notify("Kapı zaten açık..", "error", 3500)
                                    end
                                else
                                    QBCore.Functions.Notify("Yeterli polis yok..", "error", 3500)
                                end
                            end
                        else
                            QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
                                if closestHouse ~= nil then
                                    if result then
                                        ...
                                        QBCore.Functions.Notify("Birşeyleri kaçırıyormuşsun gibi görünüyor...", "error", 3500)
                                    end
                                end
                            end, "screwdriverset")
                        end
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = {name = QBCore.Shared.Items["lockpick2"]["name"], image = QBCore.Shared.Items["lockpick2"]["image"]},
    }
    while true do
        inRange = false
                        ...
                        if dist <= 1.5 then
                            ...
                            if CurrentCops >= Config.MinimumHouseRobberyPolice then
                                if Config.Houses[k]["opened"] then
                                    DrawText3Ds(Config.Houses[k]["coords"]["x"], Config.Houses[k]["coords"]["y"], Config.Houses[k]["coords"]["z"], '~g~E~w~ - Eve Gir')
    ...
    while true do
        ...
        if inside then
            if #(pos - vector3(Config.Houses[currentHouse]["coords"]["x"] + POIOffsets.exit.x, Config.Houses[currentHouse]["coords"]["y"] + POIOffsets.exit.y, Config.Houses[currentHouse]["coords"]["z"] - Config.MinZOffset + POIOffsets.exit.z)) < 1.5 then
                DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + POIOffsets.exit.x, Config.Houses[currentHouse]["coords"]["y"] + POIOffsets.exit.y, Config.Houses[currentHouse]["coords"]["z"] - Config.MinZOffset + POIOffsets.exit.z, '~g~E~w~ - Evden ayrıl')
                if IsControlJustPressed(0, 38) then
                ...
                if #(pos - vector3(Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset)) < 1 then
                    if not Config.Houses[currentHouse]["furniture"][k]["searched"] then
                        if not Config.Houses[currentHouse]["furniture"][k]["isBusy"] then
                            ...
                            DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, 'Aranıyor..')
                        end
                    else
                        DrawText3Ds(Config.Houses[currentHouse]["coords"]["x"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["x"], Config.Houses[currentHouse]["coords"]["y"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["y"], Config.Houses[currentHouse]["coords"]["z"] + Config.Houses[currentHouse]["furniture"][k]["coords"]["z"] - Config.MinZOffset, 'Bos..')
                    end
                end
            end
        end
